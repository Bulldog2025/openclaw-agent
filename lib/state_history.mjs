...
export function appendSentHistoryEntries(entries) {
  if (!Array.isArray(entries) || entries.length === 0) return 0;
  fs.mkdirSync("state", { recursive: true });
  const lines = entries.map((e) => JSON.stringify(e)).join("\n") + "\n";
  fs.appendFileSync(historyPath(), lines, "utf8");
  return entries.length;
}

export function sentHistoryHasRunId(runId) {
  try {
    const text = fs.readFileSync(SENT_HISTORY_PATH, "utf8");
    const lines = text.split("\n").filter(Boolean);

    for (const line of lines) {
      try {
        const obj = JSON.parse(line);
        if (obj?.runId === runId) return true;
      } catch {
